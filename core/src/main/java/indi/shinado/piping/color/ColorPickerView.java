...
public class ColorPickerView extends View {
	...
	public final static int SPLIT_WIDTH = 24;
	...
	private void init() {
		...
		mLeftBitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.color_press);
		mLeftBitmapRadius = mLeftBitmap.getWidth() / 2;
		mLeftSelectPoint = new PointF(SPLIT_WIDTH, SPLIT_WIDTH);
		
		mRightBitmap = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.saturation_press);
	...
	public void setSelectedColor(PointF left, PointF right){
		mLeftSelectPoint = left;
		mRightSelectPoint = right;
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mChangedListener != null) {
				mChangedListener.onColorChanged(mCallBackColor, mLeftSelectPoint, mRightSelectPoint);
				//mRightPaint.getColor(), (mRightSelectPoint.y - SPLIT_WIDTH) / (mHeight - 2 * SPLIT_WIDTH)
			}
			break;
		case MotionEvent.ACTION_UP:
			...
			if (mChangedListener != null) {
				mChangedListener.onColorChanged(getRightColor(mRightSelectPoint.y - SPLIT_WIDTH), 
						mLeftSelectPoint, mRightSelectPoint);
			}
		}
		return true;
	}
	...
	public interface OnColorChangedListener {
		void onColorChanged(int color, PointF left, PointF right);
	}
}
